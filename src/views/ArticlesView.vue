...
const loadArticles = async () => {
  ...
  const promises = Object.entries(modules).map(async ([path, module]) => {
      ...
      <div class="space-y-8">
        ...
        >
          <div 
            ...
            :key="article.path" 
            class="group flex flex-col sm:flex-row w-full items-stretch gap-6 rounded-2xl sm:shadow-[0_0px_1.2px_rgb(140,140,140)] shadow-[0_0px_2px_rgb(140,140,140)] hover:shadow-[0_0px_2px_rgb(140,140,140)] p-3 opacity-90 hover:opacity-100 hover:bg-gray-900/30 transition-all duration-300 cursor-pointer"
          ...
          >
            ...
            <div class="flex flex-col justify-between flex-grow p-4">
              <div>
                <h2 class="mb-2 font-bold text-lg group-hover:text-slate-300 transition-colors duration-300">{{ article.frontmatter.title }}</h2>
              ...
              <p class="text-sm mt-2 text-gray-400 group-hover:text-gray-300 transition-colors duration-300">{{ article.frontmatter.description || truncateContent(article.content) }}</p>
            </div>
          </div>
        </TransitionGroup>
      </div>
